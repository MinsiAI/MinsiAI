...
后台管理接口统一遵守 `minsi-admin/references/admin-rules.md`，当前后台 MVP 允许：

```txt
POST  /api/admin/auth/email/start
POST  /api/admin/auth/email/verify
...
safety_events                 必建
consent_records               必建
admin_users                   后台 MVP 允许
admin_audit_logs              后台 MVP 允许
